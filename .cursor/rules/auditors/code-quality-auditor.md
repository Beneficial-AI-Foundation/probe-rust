...
- **P11 (is-public-api from cargo-public-api)**: Verify `is-public-api` is set only under `--with-public-api` (absent otherwise), and that the candidate-form passes and their guards match P11's text (`public_api.rs`)
- **P12 (retired)**: Verify no binary-crate special-casing of `is-public-api` has crept back in; `is_library_crate` only labels the extract summary
...
- **P20 (Charon candidate resolution)**: Verify the multi-candidate filter pipeline (same-file → self-type → dedup → strict-max span), the three outcomes (Match / Ambiguous clears RQN and stamps nothing / NoMatch keeps heuristic RQN), Manifest fail-closed single-candidate validation, and object-form literal types in `format_type`
