...
| `--translation <PATH>` | | Path to an Aeneas `translation.json`. When given, `charon-def-id`/`charon-version` are read from the manifest — charon already ran once inside Aeneas, so **no charon run happens**. Implies charon enrichment and takes precedence over `--with-charon`'s LLBC path. `rust-qualified-name` stays SCIP-derived (the manifest is used only for the `def_id` join). |
