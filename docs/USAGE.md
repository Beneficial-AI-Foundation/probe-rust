...
| `--translation <PATH>` | | Path to an Aeneas `translation.json`. When given, only `charon-def-id`/`charon-version` are read from the manifest — charon already ran once inside Aeneas, so **no charon run happens**. Takes precedence over `--with-charon`: if both are set the manifest is used and the LLBC path is skipped (there is **no fallback** to a charon run if the manifest is unreadable — enrichment is simply skipped with a warning). `rust-qualified-name` and `is-public` stay SCIP-derived (the manifest is used only for the `def_id` join). |
