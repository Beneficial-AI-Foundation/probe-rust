...
**Candidate resolution** — The process of matching one [atom](#atom) against the Charon entries sharing its [match key](#match-key). Multi-candidate keys are narrowed by successive filters: same normalized file → [self-type](#self-type) match → dedup on `(def_id, qualified_name)` → [span disambiguation](#span-disambiguation). Three outcomes: **Match** (enrich; the LLBC source overrides the RQN, the [Manifest source](#manifest-enrichment-source) stamps only [charon-def-id](#charon-def-id)), **Ambiguous** (distinct same-file candidates no signal can split: nothing is stamped, and the LLBC source additionally clears the atom's [RQN](#rqn-rust-qualified-name); the Manifest source leaves it), **NoMatch** (no candidate confirmable to the atom's file, or every survivor has a usable span and none overlaps the atom: the heuristic RQN is kept; a mix of one excluding span and one span-less survivor is Ambiguous, not NoMatch). One lenient exception: a lone [LLBC-source](#llbc-enrichment-source) candidate with no file path at all is accepted on match key alone (span-less compiler-generated items); the [Manifest source](#manifest-enrichment-source) rejects it. Never an arbitrary pick. See [P20](properties.md#p20--charon-candidate-resolution-never-picks-arbitrarily), `charon_names.rs` (`resolve_charon_candidate`).
...
**File proof** — A Charon candidate's non-empty source path that normalizes (`normalize_source_path`) to the [atom's](#atom) normalized `code-path`; the only file-level evidence that a candidate can be the atom. Span overlap is a second, line-level signal on top of it. The [Manifest source](#manifest-enrichment-source) rejects any lone candidate without file proof; the [LLBC source](#llbc-enrichment-source) accepts a lone candidate that carries no file path at all (a wrong-file path is rejected by both). See `charon_names.rs` (`in_atom_file`).
...
**Library crate** — A Cargo package that has a `[lib]` target. `is_library_crate` only labels the extract summary; [is-public-api](#is-public-api) is computed identically for library and [binary-only crates](#binary-only-crate) (only under `--with-public-api`, [P11](properties.md#p11--is-public-api-from-cargo-public-api); [P12](properties.md#p12--retired-0110) is retired).

**LLBC (enrichment source)** — The `--with-charon` enrichment path: probe-rust runs [Charon](#charon) itself, parses the LLBC's `item_names`/`fun_decls`/`trait_impls`, and on a Match overrides the atom's [RQN](#rqn-rust-qualified-name) and `is-public` (when the LLBC entry carries visibility) and stamps [charon-def-id](#charon-def-id)/`charon-version`. The only source that writes (and, on Ambiguous, clears) the RQN. Counterpart of the [Manifest source](#manifest-enrichment-source), and called *legacy* because it is slated for retirement once every Aeneas project ships a `translation.json` (`resolve_enrichment` is the single dispatch point between the two). Failure is non-fatal ([P15](properties.md#p15--charon-non-fatal)). See `charon_names.rs` (`parse_llbc_names`, `EnrichmentSource::Llbc`).

**Loop helper (Manifest)** — In an Aeneas `translation.json`, a loop body that Aeneas extracted into its own `functions[]` entry. Loop helpers carry their parent function's `def_id` and a span inside the parent's, so several manifest entries can name one function; [candidate resolution](#candidate-resolution)'s dedup on `(def_id, qualified_name)` collapses them before the span stage. See `charon_names.rs` (`Enrichment::from_translation_json`).
...
**RQN (Rust Qualified Name)** — The `rust-qualified-name` field on [atoms](#atom). A `::` separated path like `crate_name::module::{Type}::method`. Derived heuristically from file path + [display name](#display-name), or precisely from [Charon](#charon) LLBC when `--with-charon` is used and [candidate resolution](#candidate-resolution) finds a Match; an atom no Charon entry matched (NoMatch) keeps the heuristic form even under `--with-charon`. The heuristic form uses bare `Type::method`; the Charon form includes `{Type}::method` with nested impl segments and may have the crate prefix from any crate in a [multi-crate LLBC](#multi-crate-llbc). Matching between Charon RQNs and atoms uses a normalized [match key](#match-key), not raw string equality. May be **absent** under `--with-charon`: [candidate resolution](#candidate-resolution) clears even the heuristic value when a Charon collision ends Ambiguous ([P20](properties.md#p20--charon-candidate-resolution-never-picks-arbitrarily) — a wrong RQN is worse than none). The [Manifest source](#manifest-enrichment-source) never replaces or clears the heuristic form; only the LLBC source does.

**Span disambiguation** — The span stage of Charon candidate resolution (P20): when multiple [Charon](#charon) candidates share the same [match key](#match-key), survivors of the file and self-type filters are compared by Charon span overlap (LLBC or manifest spans alike) with the [atom's](#atom) source line range, and only a strict-maximum positive overlap wins. Overlap is the inclusive shared line count `min(atom_end, c_end) - max(atom_start, c_start) + 1`, so a single-line span (common for dependency crates in [multi-crate LLBCs](#multi-crate-llbc)) inside the atom scores `1`. Trait type arguments are not compared: same-self-type impls are split by span alone. A tie, or missing span data on every survivor, leaves the collision ambiguous — nothing is stamped and (LLBC source) the atom's RQN is cleared rather than guessed. When every survivor carries a usable span and none overlaps the atom, the stage ends NoMatch instead (e.g. an [inherited default trait method](#inherited-default-trait-method)'s body vs. its impls further down the file) and the heuristic RQN is kept. A span-less survivor beside one positively overlapping survivor does not block it: the overlapping candidate wins (Match). See `charon_names.rs` (`span_overlap`, `resolve_charon_candidate`).
