...
A crate's *own* blanket impl (`impl<T> Trait for T`) does produce an atom, keyed after the impl's generic parameter (`…impl#[T][Trait]method()`), which `cargo public-api` reports as the bare entry `T::method`; these are resolved by [impl-descriptor resolution](#impl-descriptor-resolution), not filtered.
...
**Candidate form** — One of the names a [cargo-public-api](#cargo-public-api) entry may be matched under during `--with-public-api` processing. Every entry starts with itself as its only candidate form and gains more from the additive passes (`pub use` rewrite, trait-default resolution); an entry is matched when any of its forms names a real atom. Held per entry in `PublicNameForms`, which is what makes [entries matched](#entries-matched-vs-atoms-marked) countable. See [P11](properties.md).
...
**Entries matched (vs atoms marked)** — The two distinct `--with-public-api` metrics. *Entries matched* counts [cargo-public-api](#cargo-public-api) entries backed by some atom (`public-api entries matched: N/M`); *atoms marked* counts atoms whose `is-public-api` is `true` (`is-public-api: N true, M false`). They differ by design: several entries can share one atom (the five macro-generated basepoint tables inheriting one trait body), and one entry can be reachable through several candidate forms. Improving one does not imply improving the other. See [P11](properties.md).

**Impl-descriptor resolution** — The last `--with-public-api` pass: for an entry no atom *name* matched, the atom that implements it is found through the SCIP impl descriptor embedded in the atom key (`…impl#[SelfType][Trait]method()`) and marked `is-public-api: true` directly. Restricted to the analyzed crate's atoms, requires a unique candidate, and requires a `syn`-verified [blanket impl](#blanket-impl) for bare `T::method` and trait-level entries. This is what reaches macro-generated impl-evidence atoms, which have no [RQN](#rqn-rust-qualified-name) to match on. See [P11](properties.md), `public_api.rs` (`resolve_unmatched_to_atoms`).

**Inherited default trait method** — A trait method with a default body that an implementing type does not override. The body exists once, in the trait; SCIP creates no per-type symbol, so `cargo public-api`'s per-type entry (`EdwardsBasepointTableRadix32::mul_base_clamped`) has no atom of its own and is matched to the trait's atom by the trait-default pass. Contrast a *required* method (no default body), whose per-type entry has no trait body to fall back to. See [P11](properties.md).
